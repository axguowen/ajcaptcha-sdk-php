...
class CacheUtils
{
    ...
    public function __construct(array $options = [])
    {
        ...
        if (empty($this->options['path'])) {
            // 获取应用运行时路径
            $runtimePath = realpath(dirname(__FILE__, 7)) . DIRECTORY_SEPARATOR . 'runtime';
            // 拼接路径
            $this->options['path'] = $runtimePath . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'ajcaptcha';
        }

        // 过滤目录分隔符
        $this->options['path'] = rtrim($this->options['path'], DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;
    }
